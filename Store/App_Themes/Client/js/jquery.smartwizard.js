...
(function($){
    $.fn.smartWizard = function(action) {
        ...
        return this.each(function(){
                ...
                function showStep(stepIdx){
                    ...
                    if(options.transitionEffect == 'slide'){
                      $($(curStep, obj).attr("href"), obj).slideUp("fast",function(e){
                            ...
                            curStepIdx =  stepIdx;                        
                            SetupStep(curStep,selStep,stepIdx);
                          });
                    ...
                    } else if(options.transitionEffect == 'slideright'){
                        var nextElmRight = 0;
                        var curElementRight = 0;
                        if(stepIdx > curStepIdx){
                            nextElmRight1 = contentWidth + 10;
                            nextElmRight2 = 0;
                            curElementRight = 0 - $($(curStep, obj).attr("href"), obj).outerWidth();
                        } else {
                            nextElmRight1 = 0 - $($(selStep, obj).attr("href"), obj).outerWidth() + 20;
                            nextElmRight2 = 0;
                            curElementRight = 10 + $($(curStep, obj).attr("href"), obj).outerWidth();
                        }
                        if(stepIdx == curStepIdx){
                            nextElmRight1 = $($(selStep, obj).attr("href"), obj).outerWidth() + 20;
                            nextElmRight2 = 0;
                            curElementRight = 0 - $($(curStep, obj).attr("href"), obj).outerWidth();                           
                        }else{
                            $($(curStep, obj).attr("href"), obj).animate({right:curElementRight},"fast",function(e){
                              $($(curStep, obj).attr("href"), obj).hide();
                            });                       
                        }

                        $($(selStep, obj).attr("href"), obj).css("right",nextElmRight1);
                        $($(selStep, obj).attr("href"), obj).show();
                        $($(selStep, obj).attr("href"), obj).animate({right:nextElmRight2},"fast",function(e){
                          curStepIdx =  stepIdx;                        
                          SetupStep(curStep,selStep,stepIdx);                      
                        });
                    } else{
                ...
                function SetupStep(curStep,selStep,stepIdx){
                   $(curStep, obj).removeClass("selected");
				   $(curStep, obj).parent().prev().find("a").css("background-position","left -68px");
				   
				   if (steps.length == stepIdx + 2) $(curStep, obj).css("background-position","left -300px");
				   	else $(curStep, obj).css("background-position","left -66px");
                   $(curStep, obj).addClass("done");
                   
                   $(selStep, obj).removeClass("disabled");
                   $(selStep, obj).removeClass("done");
                   $(selStep, obj).addClass("selected");
				   $(selStep, obj).css("background-position","left -2px");
				   $(selStep, obj).parent().prev().find("a").css("background-position","left -201px");
				   if (steps.length == (stepIdx + 1)) { $(selStep, obj).css("background-position","left -300px"); }
    ...
    $.fn.smartWizard.defaults = {
          ...
          keyNavigation: true, // Enable/Disable key navigation(right and left keys are used if enabled)
          enableAllSteps: false,
          updateHeight: true,
          transitionEffect: 'fade', // Effect on navigation, none/fade/slide/slideright
