 ...
 *
 * Copyleft 2012-2013 Dimox (http://dimox.name/)
...
(function($) {
	$.fn.styler = function(opt) {
		...
		return this.each(function() {
			...
			// checkbox
			if (el.is(':checkbox')) {
				el.css({position: 'absolute', right: -9999}).each(function() {
			...
			// file
				...
				el.css({position: 'absolute', top: '-50%', left: '-50%', fontSize: '200px', opacity: 0}).each(function() {
					if (el.parent('span.jq-file').length < 1) {
						...
						var name = $('<div class="jq-file__name" style="float: right; white-space: nowrap"></div>').appendTo(file);
							...
							function doSelect() {
								var selectbox =
									$('<span' + id + ' class="jq-selectbox jqselect' + cl + '" style="display: inline-block; position: relative; z-index:' + opt.singleSelectzIndex + '">'+
											'<div class="jq-selectbox__select" style="float: right"><div class="jq-selectbox__text"></div>'+
												'<div class="jq-selectbox__trigger"><div class="jq-selectbox__trigger-arrow"></div></div>'+
											'</div>'+
										'</span>');
								el.after(selectbox).css({position: 'absolute', right: -9999});
